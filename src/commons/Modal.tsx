    ...
    >
      <div
        className={`
          ...
          ${title ? "min-h-[12rem]" : "min-h-[10.5rem]"}  /* 12rem = 192px, 10.5rem = 168px */
          gap-7
        `}
      ...
      >
        {/* 모달 제목 */}
        {title && (
          <h2 className="text-sm-medium">  {/* 14px Medium - global.css의 text-sm-medium 클래스 사용 (#A3A08F) */}
            {title}
          </h2>
        )}
        ...
        {/* 모달 콘텐츠 */}
        {description ? (
          <p className="text-center text-title-lg">  {/* 18px Medium - global.css의 text-title-lg 클래스 사용 (#2E2E2C) */}
            {description}
          </p>
        ) : (
          children
         ...
         * - 높이: 3rem (48px) 고정 (h-12)
         */}
        <div className={`w-full mt-auto ${hasCancel ? "flex gap-2.5" : ""}`}>  {/* gap-2.5 = 10px */}
          {hasCancel && onClose && (
